namespace MyLinq.Tests
{
    public class LinqTests
    {
        ...
        public void Chunk()
        {
            ...
            var chunked = list.Chunk(4);

            var group1 = chunked.First();
            var group2 = chunked.Last();

            Assert.Equal(4, group1.Count());
            Assert.Equal(4, group2.Count());
        }

        [Fact]
        public void GetPreviousOrDefault()
        {
            var list = new List<TestObject<float>>();
            var obj = new TestObject<float> { Height = 1, Width = 3.5f, Depth = 8, Qty = 1 };
            list.Add(obj);
            ...
            var prev1 = list.GetPreviousOrDefault(obj);
            var prev2 = list.GetPreviousOrDefault(obj2);

            Assert.Null(prev1);
            Assert.Equal(obj, prev2);
        }

        [Fact]
        public void GetNextOrDefault()
        {
            var list = new List<TestObject<float>>();
            var obj = new TestObject<float> { Height = 1, Width = 3.5f, Depth = 8, Qty = 1 };
            list.Add(obj);
            ...
            var next1 = list.GetNextOrDefault(obj);
            var next2 = list.GetNextOrDefault(obj2);

            Assert.Null(next2);
            Assert.Equal(obj2, next1);
        }
    }
}